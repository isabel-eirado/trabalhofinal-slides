...
### Gráfico 1 - Comentários ####

- A partir do gráfico, percebe-se que:
1. O Alabama é um outlier em relação ao total de palavras, na medida em que dispõe de um total de palavras maior do que qualquer outro estado. Por isso, para fins estatísticos, foi excluído de algumas das análises posteriores. 
...
- Ao investigar de que forma esse conjunto de dados está variando, precebe-se que a Califórnia é o Estado que apresenta a maior variação em relação a média de palavras totais. Enquanto estados como Alaska, Minnesota, Wisconsin, Vermont, Indiana e South Dakota parecem ser os estados com o menor número de mudanças constitucionais.
...
- O T teste é um teste estatístico que tem valores entre 0 e 1 e tem como finalidade a comparação das igualdades entre duas médias. O p valor obtido pelo teste é igual a 0.0009796, o que sugere a rejeição da igualdade (h0) entre as médias em t0 e t1.
...
- Objetivamos identificar a quantidade de constituições totais que cada estado da Federação teve e verificar o grau de mudanças constitucionais a partir desse valor.
...
### Gráfico 4 - Comentários

- A partir do Gráfico percebemos que:
1. A partir do Gráfico percebemos que Louisiana é o Estado com o maior número de constituições.
2. Diversos outros estados apresentam apenas duas constituições ao longo do tempo, indicando menos mudanças constitucionais. São eles: 

```{r paged.print=TRUE, echo=FALSE, message = FALSE}
const_eua_painel %>% filter(numero_total_constituicoes ==2)
```

3. Enquanto a Califórnia figura como o estado com o maior desvio padrão entre as médias observadas nas estados, isto não se observa no quantitativo das mudanças constitucionais. O estado tem uma posição mediana, mudando suas normas constitucionais apenas 4 vezes. 
---

### Gráfico 5 - Relação entre número de Constituições por estado e data de criação do estado ####

- Objetivamos identificar se há alguma relação entre a criação do estado e o número de constituições promulgadas, isto é, se a data de criação do estado também influencia nas mudanças constitucionais.
...
- Dada a dispersão entre os pontos do gráfico, não foi possível notar relação entre a data de criação do estado e as mudanças constitucionais. 
...
### Criando o Índice de Mudança Proporcional dos Poderes
...
- Dado o alpha de aproximadamente 0,83, conclui-se, a partir do teste, que há uma boa relação entre as variáveis relativas os Três Poderes.  
